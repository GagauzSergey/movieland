...
ALTER TABLE `movieland`.`movie`
CHANGE COLUMN `votes` `votes` INT(11) NOT NULL DEFAULT '100' ;

ALTER TABLE `movieland`.`user`
ADD COLUMN `type` VARCHAR(1) NOT NULL DEFAULT 'U' AFTER `password`;

ALTER TABLE `movieland`.`movie_country`
DROP FOREIGN KEY `movie_country_movie_id`;
ALTER TABLE `movieland`.`movie_country`
ADD CONSTRAINT `movie_country_movie_id`
  FOREIGN KEY (`movie_id`)
  REFERENCES `movieland`.`movie` (`id`)
  ON DELETE CASCADE
  ON UPDATE NO ACTION;

ALTER TABLE `movieland`.`movie_genre`
DROP FOREIGN KEY `movie_id`;
ALTER TABLE `movieland`.`movie_genre`
...
ALTER TABLE `movieland`.`rating`
DROP FOREIGN KEY `rating_movie_id`;
ALTER TABLE `movieland`.`rating`
...
DROP FOREIGN KEY `review_movie_id`;
ALTER TABLE `movieland`.`review`
